...
- July 7, 2025: Completely rewrote determinePlanetType function with realistic astronomical zone logic. Now uses scaled AU orbit radius (orbitRadius/6) with star temperature factors - hotter stars push habitable zones outward, colder stars pull them inward. Planet types are now properly ordered by distance: nuclear/barren/arid worlds closest to star, then sandy/dusty/martian worlds, followed by habitable jungle/marshy/grassland/ocean worlds, then cold tundra/snowy worlds, and finally gas/frost giants and methane worlds in outer system. Fixed gas/frost giant overabundance by extending to 6-zone system with better terrestrial world distribution in cold regions.
- July 7, 2025: Implemented automatic music track progression system. Added second soundtrack (galacticview2_roguestars.mp3) that automatically plays after the first track ends. Music now progresses through multiple tracks seamlessly without manual intervention, using the existing useAudio store infrastructure for track management.
- July 10, 2025: Implemented warp lane network system in galactic view using Dijkstra's pathfinding algorithm. Added WarpLane interface to schema for future editing capabilities. System generates 8-10 warp lanes between distant star pairs (minimum 1/2 galaxy radius apart) and visualizes them as translucent colored cylinders tracing the shortest paths between stars. Each warp lane uses distinct colors (cyan, magenta, yellow, green, etc.) and shows multi-hop routes through intermediate stars.
- July 23, 2025: Completed comprehensive army movement and control system in planetary view. Armies are properly positioned on planetary sphere surface using spherical coordinates, grouped with surface features for synchronized rotation with planet. Implemented point-and-click movement controls: select army (orange highlighting), click planet surface to move with smooth animation. Planet rotation automatically stops when army is selected for precise targeting, resumes after movement order is given.
- July 23, 2025: Fixed space feature display crashes by resolving React state management issues and adding proper null checking. Ship graveyards now properly display affiliation instead of undefined faction data. Added comprehensive error handling throughout the space feature click chain.
- July 23, 2025: Implemented detailed 3D geometry for space features. Space stations feature cylindrical hubs with rotating torus rings, mining stations have cubic processing units with extending cylindrical arms, orbital defenses use octahedral platforms with cone turrets, ship graveyards display scattered debris using dodecahedron and box geometries, and research stations feature icosahedral cores with extending research arrays. Each feature type has unique visual identity with appropriate materials and emissive properties.
- July 23, 2025: Enhanced orbital defense platforms with additional small cannons positioned around the main platform for better defensive coverage. Reduced all space feature sizes by 50% for better visual proportion and less visual clutter in system view. Defense platforms now feature 4 small cylindrical cannons extending in cardinal directions plus main top/bottom turrets.
- July 23, 2025: Performed deep camera state analysis and resolved camera conflicts. Identified and fixed camera control infighting between CameraController and PlanetaryView components. CameraController now properly respects the disableGalacticSystemControls flag in both useFrame loop and mouse event handlers, preventing competing camera systems from fighting for control. Mouse events, keyboard controls, and orbital tracking are properly disabled when PlanetaryView takes camera control.
- July 23, 2025: Fixed space feature spawning issues by lowering technology requirements (space stations: ≥7 to ≥5, orbital defenses: ≥3), improving mining station generation to use any faction if no advanced ones exist, increasing mining station spawn chance to 80%, and adding missing research station generation code entirely. Added comprehensive debug logging and fixed space feature UI persistence - selectedSpaceFeature now properly clears when switching scopes (Escape key, Backspace navigation, Enter transitions) just like planet selection UI.
- July 24, 2025: Enhanced army positioning and movement system: armies now spawn 1.0-2.5 units away from features instead of directly on them, preventing overlap with surface features. Improved division visibility with larger formation radius (0.15-0.23 units from army), increased division size and height, and bright green coloring for better distinction. Fixed coordinate transformation for accurate army movement relative to planet surface rotation. Added comprehensive debug logging for army and division tracking during movement animations.
- July 24, 2025: Implemented comprehensive fleet and ship system for system view with realistic orbital mechanics. Fleet generation creates 3-8 ships per faction homeworld fleet based on technology level. Ships orbit in formations around fleet command ships, maintaining proper orbital speeds matching planetary mechanics. Point-and-click fleet movement uses invisible orbital disk for target selection. Fleet markers use octahedral command ships with surrounding ship formations (cyan ship markers). Fleet movement features smooth interpolation and automatic orbital motion around stars. System includes ship types from fighter wings to super carriers with appropriate sizing and formation positioning.
- July 24, 2025: Enhanced fleet orbital mechanics to use exact planetary rotation formula (Math.sqrt(1 / orbitRadius) * 0.15) for consistent orbital speeds. Added FleetInfoPanel and ArmyInfoPanel UI components with faction-colored borders and detailed information display. Fleet panels show ship composition, orbit radius, and technology level. Army panels display division count, total units, and movement instructions. Panels automatically appear when units are selected and can be closed manually.
- July 24, 2025: Updated ship orbital mechanics to match fleet/planet formula for consistent movement speeds. Ships now orbit their fleets using the same Math.sqrt(1 / orbitRadius) * 0.15 calculation with staggered positioning. Added comprehensive error handling to prevent undefined access crashes in both FleetInfoPanel and ArmyInfoPanel components. All THREE.js object access and HTML data rendering now includes proper null checks and fallback values.
- July 24, 2025: Fixed fleet spawning system to position fleets near faction homeworld planets instead of random star orbits. Fleets now spawn 0.8-0.9x the homeworld's orbital radius with a 30-60 degree angular offset from the planet. Implemented continuous ship position updates using real-time fleet position tracking to ensure ships maintain formation regardless of fleet selection status. Enhanced ship rotation system to create realistic fleet formations - all ships maintain parallel orientation and rotate with orbital curvature, facing the direction of fleet movement for authentic military formation appearance.
- July 24, 2025: Implemented comprehensive fleet movement system with dual-layer orbital disk design. Created invisible clickable plane positioned above celestial objects (y=10) for reliable click detection, and separate wireframe circle below (y=-5) for visual feedback. Fixed fleet position updates using direct mutation of system data. Fleet movement now works with point-and-click interface using subtle cyan wireframe guide that doesn't interfere with interactions.
- July 24, 2025: Enhanced fleet orbital mechanics with optimized calculations and proper phase distribution. Added orbitCenter field to Fleets schema for flexible orbital reference points. Implemented cached orbit radius calculation using useMemo to prevent drift from recalculating moving positions. Added random phase offsets (initialAngle) to prevent all fleets from orbiting in perfect synchronization. Fixed coordinate transformation issues in fleet movement by removing rotational transforms and using horizontal plane geometry for accurate click detection.
- July 25, 2025: Fixed fleet movement system with pause-and-resume orbital mechanics. Fleets now stop orbital motion when moved, hold position exactly at clicked coordinates for 1 second, then resume orbiting from new position. Implemented movement state tracking with useEffect to detect position changes. Fixed coordinate system by positioning orbital disk at fleet height with proper horizontal orientation (-Math.PI/2 rotation) for accurate XZ plane click detection. Synchronized fleet orbital speeds with planetary mechanics - fleets now use exact same orbital speed formula as planets (Math.sqrt(1/orbitRadius) * 0.15) with proper logical/visual radius scaling to match planetary orbital behavior. Fixed ocean.jpg texture loading error with proper error handling. Resolved fleet orbital radius mismatch by implementing identical scaling system as planets: logical radius for speed calculations, visual radius (2x logical) for positioning. Fleet generation now uses orbitRadius * 2 for positioning to match planet visual scaling. Implemented fleet orbital speed "cheat" system where fleets adopt the orbital speed of the closest planet by orbital radius for perfect synchronization. Simplified ship orientation to face acceleration direction, naturally handling orbital curvature without complex rotation calculations.
- July 27, 2025: Fixed AU distance calculation in fleet information systems. Implemented consistent position-based orbital radius calculation (sqrt(x² + z²) / 6) across View Fleets menu and FleetInfoPanel. Fleet AU distances now display correctly in both listing view and detailed information panels, matching actual fleet positions in 3D space.
- July 27, 2025: Fixed critical fleet position reset bug when returning from planetary to system view. Added fleet position restoration to Backspace key handler that was missing this logic. Fleets now maintain their moved positions correctly when transitioning between planetary and system views, preventing reset to original spawn locations.
