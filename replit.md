...
- July 25, 2025: Fixed fleet movement system with pause-and-resume orbital mechanics. Fleets now stop orbital motion when moved, hold position exactly at clicked coordinates for 1 second, then resume orbiting from new position. Implemented movement state tracking with useEffect to detect position changes. Fixed coordinate system by positioning orbital disk at fleet height with proper horizontal orientation (-Math.PI/2 rotation) for accurate XZ plane click detection. Synchronized fleet orbital speeds with planetary mechanics - fleets now use exact same orbital speed formula as planets (Math.sqrt(1/orbitRadius) * 0.15) with proper logical/visual radius scaling to match planetary orbital behavior. Fixed ocean.jpg texture loading error with proper error handling.
