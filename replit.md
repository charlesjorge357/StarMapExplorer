...
- July 7, 2025: Implemented realistic planet count distribution based on stellar class research. G-class (Sun-like) stars now have 8-10 planets minimum, M-class red dwarfs have 1-7 planets (with TRAPPIST-1 style 7-planet systems at 15% rate), K-class have 4-8 planets, F-class have 6-9 planets, and large A/B/O-class stars can have up to 8 planets. Planet count is now independent of stellar size but appropriate to stellar type.
- July 7, 2025: Fixed planet mass calculation with realistic density factors by planet type. Gas giants now use 0.25 density (Jupiter-like), frost giants 0.3 (Neptune-like), nuclear worlds 1.8 (heavy elements), ice worlds 0.6, ocean worlds 0.9, Earth-like worlds 1.0, and barren/Mars-like worlds 0.7. Mass now properly correlates with radius and composition instead of varying wildly independent of planet type.
- July 7, 2025: Fixed orbital distance display and planet type scaling with consistent AU conversion factor. Applied division by 6 to both UI display and planet type calculation logic while maintaining correct 3D visual positioning. Planets now show realistic AU values (0.4-10 AU range) and use proper zone boundaries (1.0 AU hot zone, 3.0 AU mid zone) for accurate planet type assignment based on stellar luminosity.
- July 7, 2025: Implemented comprehensive 8-zone orbital system for enhanced planet type distribution. Zones include: Scorched (0-0.3 AU, nuclear/barren worlds), Hot (0.3-0.7 AU, Mercury-like), Warm (0.7-1.2 AU, potentially habitable), Habitable (1.2-2.0 AU, Earth-like), Temperate (2.0-3.5 AU, Mars-like), Cool (3.5-6.0 AU, gas giants), Cold (6.0-12.0 AU, frost giants), and Frozen (12.0+ AU, outer ice worlds). Each zone has appropriate planet type probabilities and gas giant spawn rates.
- July 7, 2025: Added stellar class-based zone scaling system. Zone boundaries now expand/contract based on stellar temperature deviation from G-class (Sun-like) baseline using square root scaling. Hot stars (A, B, O classes) push zones outward for realistic habitable zone positioning, while cool stars (K, M classes) pull zones inward. Uses separate realistic AU progression (Mercury=0.39 AU, Venus=0.72 AU, etc.) independent of visual orbital distances.
- July 7, 2025: Implemented type-first planet generation system replacing orbit-determines-type approach. System now randomly selects planet types with bias against duplicates, then places each type in appropriate orbital zones. Fixed planet generation methods to properly use overrideType parameter, resolving issue where all planets defaulted to 'arid_world'. Enhanced logging for better debugging of planet generation process.
- July 7, 2025: Replaced complex zone system with simple hardcoded orbital mapping. Each planet type now has fixed AU distance options (nuclear worlds: 0.3-0.5 AU, gas giants: 4.5-11.0 AU, frost giants: 12.0-20.0 AU, habitable worlds: 1.0-2.5 AU). Eliminated stellar temperature-based zone scaling for reliable and predictable planet placement.
- July 7, 2025: Reduced planet counts significantly to prevent overcrowding and improve orbital placement accuracy. M-class stars: 1-4 planets (was 1-7), G-class stars: 3-6 planets (was 8-10), increased minimum orbital separation from 12 to 18 units. Systems now have proper spacing allowing planets to occupy their designated orbital zones.
